...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* theme_setup */
if ( !function_exists( 'jawsdays2014_theme_setup' ) ) :
	add_action( 'after_setup_theme', 'jawsdays2014_theme_setup' );

remove_action( 'wp_head', 'jetpack_og_tags' );

function jawsdays2014_theme_setup() {
	global $content_width;

	if ( !isset( $content_width ) )
		$content_width = 744;

	load_theme_textdomain( 'jawsdays2014', get_template_directory() . '/languages' );
	...
	$defaults = array(
		...
		'width'         => apply_filters( 'jawsdays2014_header_image_width', 1500 ),
		'height'        => apply_filters( 'jawsdays2014_header_image_height', 479 ),
		'header-text'   => false,
	);
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/**
 * jawsdays2014_wp_title
 */
add_filter( 'wp_title', 'jawsdays2014_wp_title', 10, 2 );
function jawsdays2014_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( get_query_var( 'news' ) && is_archive() ) {
		$label_name = __( 'What\'s new', 'jawsdays2014' );
		$title = $label_name . $sep . $title;
	}

	// Add a page number if necessary.
	if ( $paged >= 2 || $page >= 2 )
		$title = sprintf( __( 'Page %s', 'jawsdays2014' ), max( $paged, $page ) ) . $sep . $title;

	return $title;
}

/**
 * jawsdays2014_head_mobile_meta
 */
add_action( 'wp_head', 'jawsdays2014_head_mobile_meta' );
function jawsdays2014_head_mobile_meta() {
	echo <<< EOT
 ...
 */
add_action( 'wp_enqueue_scripts', 'jawsdays2014_scripts_styles' );
function jawsdays2014_scripts_styles() {
	...
	wp_enqueue_style( 'jawsdays2014-fonts', add_query_arg( $query_args, "$protocol://fonts.googleapis.com/css" ), array(), null );

	wp_enqueue_style( 'jawsdays2014-style', get_template_directory_uri() . '/style.css' , array(), get_file_time( 'style.css' ) );

}

add_action( 'jawsdays2014_body_before', 'add_fb_root' );
...
/* Header
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* jawsdays2014_main_nav */
function jawsdays2014_main_nav() {
	echo get_jawsdays2014_main_nav();
}

/* get_jawsdays2014_main_nav */
function get_jawsdays2014_main_nav() {
	$output = wp_nav_menu( array( 'menu' => 'main-menu', 'container_id' => 'main-nav-box', 'echo' => false, 'theme_location' => 'main_menu' ) );
	return $output;
}

/* jawsdays2014_main_img */
function jawsdays2014_main_img() {
	echo get_jawsdays2014_main_img();
}

/* get_jawsdays2014_main_img */
function get_jawsdays2014_main_img() {
...
function get_entry_more_link( $post_id = null ) {
	...
	return '<p class="entry-more"><a href="'. get_permalink( $post_id ) . '">' . __( 'Read more &raquo;', 'jawsdays2014' ) . '</a></p>';
}
...
/* Archive
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* jawsdays2014_page_title */
function jawsdays2014_page_title() {
	echo get_jawsdays2014_page_title();
}

/* get_jawsdays2014_page_title */
function get_jawsdays2014_page_title() {
	global $wp_query;
	$output = '';
	if ( !is_home() || !is_front_page() ) {
		...
		elseif ( is_post_type_archive() ) :
			...
			$output .= __( 'Archives', 'jawsdays2014' );
		endif;
	}
	return $output;
}
...
/* Single
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
function jawsdays2014_link_pages() {
	echo get_jawsdays2014_link_pages();
}

function get_jawsdays2014_link_pages() {
...
function jawsdays2014_content_nav() {
	echo get_jawsdays2014_content_nav();
}
function get_jawsdays2014_content_nav() {
	global $wp_query;
	$output = '';
...
/* HOME
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
function jawsdays2014_slide( $limit = -1 ) {
	//echo get_jawsdays2014_slide( $limit );
}
function get_jawsdays2014_slide( $limit = -1 ) {
	$output = '';
	$posts_array = array();
	$args = array(
		'post_type' => 'sponsor',
...
function jawsdays2014_new_post( $args = array() ) {
	echo get_jawsdays2014_new_post( $args );
}
function get_jawsdays2014_new_post( $args = array() ) {
	$output = '';
	$posts_array = array();
	$defaults = array(
		'posts_per_page' => 2,
	...
	$output .= '<h2 class="new-post-title">' . esc_html__( 'News', 'jawsdays2014' ) . '</h2>' . "\n";
	foreach ( $posts_array as $post ) {
		setup_postdata($post);
		$id = $post->ID;
	...
	$output .= '<p class="more-archive-list"><a href="' . esc_url( home_url('news/') ) . '">' . esc_html__( 'Archive list &gt;', 'jawsdays2014' ) . '</a></p>' . "\n";
	$output .= '</div>' . "\n";
	return $output;
}
...
/* Footer
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* jawsdays2014_footer_nav */
function jawsdays2014_footer_nav() {
	echo get_jawsdays2014_footer_nav();
}

/* get_jawsdays2014_footer_nav */
function get_jawsdays2014_footer_nav() {
	$output = wp_nav_menu( array( 'menu' => 'footer-menu', 'container_id' => 'footer-nav-box', 'echo' => false, 'theme_location' => 'footer_menu' ) );
	return $output;
}

function jawsdays2014_social_button() {
	echo get_jawsdays2014_social_button();
}
function get_jawsdays2014_social_button() {
...
add_action( 'wp_footer', 'jawsdays2014_footer_script', 99 );
function jawsdays2014_footer_script() {
	echo <<< EOT
